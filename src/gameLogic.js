...
export function getEndingVariant({
  ...
  const seasonPressure = Math.max(closingPressure, peakRiskPressure);
  ...
  const freeTextCount = log.filter((entry) => entry?.freeTextSuccess).length; const lowerPriorityEndingsOpen = seasonPressure < 31 && humanCost < 90 && discoveredClues.length < 4 && freeTextCount < 2; if (lowerPriorityEndingsOpen && capital >= 55 && trust < 48) return { id: "profitable-silence", label: "PROFITABLE SILENCE", title: "The organization survived, but the question was sealed.", text: "Profit arrived with a record of what was left unsaid.", failure: false }; if (lowerPriorityEndingsOpen && legitimacy >= 60 && trust < 55) return { id: "cold-justice", label: "COLD JUSTICE", title: "The procedure held, but the person inside it did not.", text: "Legitimacy was preserved while the relationship cost moved forward.", failure: false }; if (lowerPriorityEndingsOpen && trust - legitimacy >= 8) return { id: "field-pact", label: "FIELD PACT", title: "The field made its promise before the approval arrived.", text: "Your relationship network opened the next locked record.", failure: false };
