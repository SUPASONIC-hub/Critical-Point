...
export function getEndingVariant({ resources = {}, discoveredClues = [], log = [] } = {}) {
  ...
  if (trust >= 65 && legitimacy < 45) return { id: "field-pact", label: "FIELD PACT", title: "공식 승인보다 먼저, 현장의 약속이 다음 문을 열었다.", text: "당신의 관계망이 잠긴 기록에 접근할 수 있게 합니다.", failure: false };
  if (pressure <= 20 && discoveredClues.length <= 1) return { id: "quiet-cover", label: "QUIET COVER", title: "위험은 낮췄지만, 진실도 아직 잠들어 있다.", text: "다음 플레이에서는 숨겨진 단서를 우선 추적해야 합니다.", failure: false };
