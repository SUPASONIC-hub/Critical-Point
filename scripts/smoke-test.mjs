...
test("previous free-text routes should add a next-case memory choice into the hidden system route", () => {
  assert.equal(
    getContinuityMemoryChoice({
      ...
      caseResults: { case12: { routeMemory: getRouteMemory([{ nodeId: "c12_route_system", choiceId: "c12_route_system_publish", freeTextSuccess: true }]) } },
    }).next,
    "f_route_system",
...
test("the final act should have a system-led opening route", () => {
  // Keyed on case 12's aftermath: that is the case the finale now follows.
  assert.equal(caseOpeningRoutes.final.c12_after_fund, "f_start_system", "the final act should have a system-led opening route");
});
test("case 08 through 12 should open from the aftermath before them", () => {
...
test("78 generated scenes should expose 240 authored choices: three each across thirteen cases", () => {
  assert.equal(
    generatedChoiceCount,
    240,
    "78 generated scenes should expose 240 authored choices: three each across thirteen cases",
  );
});
