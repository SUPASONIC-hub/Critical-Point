...
src/main.jsx           화면, 상태 관리, 음악 토글, 저장/이어하기, 케이스 전환, 선택 타이머, 접힘 정보 서랍, 전술 등급/보너스 UI
...
`npm test`는 자원 변화, 위험 압력, 모멘텀/랭크 계산, 개인정보 감지/익명화, 텍스트 제한 로직을 확인합니다. 선택 타이머는 장면을 강제로 종료하지 않으며, 12초 안에 챌린지를 맞히면 `QUICK READ` 보너스가 적용됩니다. 위험 압력이 60 이상이면 시간·현금을 추가로 소모해 정당성을 회복하는 `위기 프로토콜`을 케이스당 한 번 사용할 수 있습니다.
