...
**45초 안에, 누가 대가를 지불할지 정한다.** 판단 스릴러입니다. 플레이어는 트리거랩의 전환
분석관이 되어 여섯 사건을 지나며, 매 장면 45초의 결정 창 안에서 무엇을 먼저 지킬지 고릅니다.
어떤 선택도 공짜가 아니고, 고른 순서는 다음 사건의 조건과 결말에 그대로 남습니다.
...
- **판단 레일** — 한 번의 결정이 한 화면에 들어갑니다. 상시로 보이는 것은 45초 결정 시계와
  시간·믿음·사람 피해 세 개의 게이지뿐이고, 목표·압박·인물·단서·규칙은 `기록실` 한 곳에
  접혀 있습니다. 자원 수치 예보는 기본으로 숨겨져 있고 `전술 정보`를 열 때만 나옵니다 --
  먼저 장면으로 판단하고, 확정 뒤 연출창에서 실제 수치를 확인하는 순서입니다.
- **보상** — 장면 목표 연속 달성(STREAK BREAKTHROUGH), 5연속 달성(PERFECT RUN), 숨은 단서
  발견(EVIDENCE BONUS)이 선택 직후 연출창에 뜹니다.
- **결말** — 자원 임계값과 누적된 선택 성향으로 갈리는 엔딩. 리포트는 3막입니다: 결말과
  랭크와 다음 사건이 첫 화면, `왜 이렇게 됐나` 세 장(결말을 정한 축, 반복한 방식, 이번에 열지
  않은 문)이 그다음, 나머지 전부는 `전체 기록` 안에 있습니다.
...
- 배경음은 사용자 제스처 이후 장면 분위기에 맞춰 재생됩니다. 절차적 스코어에 백비트는
  없습니다 -- 맥박 같은 킥과 방의 소음만 남기고, 결정 창의 마지막 10초에는 초당 하나씩
  심박이 붙습니다. 음소거와 볼륨 프리셋은 `acquireCueRuntime()` 한 곳에서 확인하며, 음소거된
  플레이어에게는 `AudioContext` 자체를 만들지 않습니다. 노드를 만들고 게인을 0으로 두는 것도
  컨텍스트 비용을 내는 일이고, 그게 음소거가 거절한 것이기 때문입니다.
